...
All package components return consumer slot nodes or `null`. They do not insert wrapper elements.
...
Use `Form` as the normal root and `FormProvider` when the name better communicates context-only composition.

```vue
<Form :form="resource" v-slot="{ form, submit, processing }">
...
`Wizard` accepts a form resource and options, creates both the form engine and wizard controller, provides the form context, and exposes `{ form, wizard }` to its default slot and component ref.
